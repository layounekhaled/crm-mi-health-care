...
async function main() {
  ...
  const formationVent = await db.event.create({ data: { nom: 'Formation Ventilateurs Löwenstein', ville: 'Alger', date: new Date('2025-12-05'), type: 'formation', marques: 'Löwenstein', equipe: 'Youcef, Nadia' } })
  const santeExpo = await db.event.create({ data: { nom: 'Santé Expo 2026', ville: 'Constantine', date: new Date('2026-03-10'), type: 'expo', marques: 'MIR,BOS,Gelenke,Yuwell', equipe: 'Sara, Amine' } })
  ...
  await db.eventProspect.create({ data: { eventId: santeExpo.id, prospectId: drKaci.id } })
  await db.eventProspect.create({ data: { eventId: santeExpo.id, prospectId: drOuali.id } })
  ...
  const opp7 = await db.opportunity.create({ data: { nomProjet: 'Prospect Tlemcen - Échographe', statut: 'Nouveau', montantEstime: 3500000, commercialId: amine.id } })
  const opp8 = await db.opportunity.create({ data: { clientId: drTaleb.id, nomProjet: 'Anesthésie Hôpital Sétif', statut: 'Perdu', montantEstime: 5000000, motifPerte: 'Concurrent choisi', commercialId: sara.id } })
  ...
  const opp10 = await db.opportunity.create({ data: { nomProjet: 'Ventilateurs CHU Batna', statut: 'Contacté', montantEstime: 7500000, commercialId: sara.id } })
  ...
  await db.operation.create({ data: { opportunityId: opp2.id, produit: 'Table Radiologie Yuwell', marque: 'Yuwell', responsableId: sara.id, prixEstime: 5000000, marge: 900000, statut: 'en_attente', priorite: 'moyenne' } })
  ...
  await db.operation.create({ data: { opportunityId: opp4.id, produit: 'Moniteur Cardiaque MIR', marque: 'MIR', responsableId: sara.id, prixEstime: 4500000, marge: 900000, statut: 'en_attente', priorite: 'moyenne' } })
  ...
  await db.afterSale.create({ data: { clientId: drBenahmed.id, type: 'livraison', statut: 'en_attente', date: new Date('2026-02-01'), employeId: youcef.id, notes: 'Livraison moniteur patient prévue février.' } })
  ...
  console.log(`  - 10 operations`)
  console.log(`  - 10 tasks`)
  console.log(`  - 5 interactions`)
  console.log(`  - 5 after-sales`)
